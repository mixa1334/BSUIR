...
class View(tk.Tk):
    ...
    def __command_user_info(self):
        ...
        (*) фильтры:\n
        - выбрать слово, которое должно встречаться в тексте
        """
        top = tk.Toplevel()
    ...
    def __command_save_as_doc(self):
        ...
        l_filter_name = tk.Label(top, text="Слово которое должно быть в предложении")
        l_filter_name.grid(row=0, column=0, pady=10, padx=10)
        e_filter_value = tk.Entry(top)
        e_filter_value.grid(row=0, column=1, padx=10, pady=10)
        l_filename = tk.Label(top, text="Имя файла")
        l_filename.grid(row=1, column=0, padx=10, pady=10)
        e_filename = tk.Entry(top)
        e_filename.grid(row=1, column=1, padx=10, pady=10)
        top.filename = e_filename
        top.filter_value = e_filter_value

        submit = tk.Button(top, text="Сохранить", command=lambda: self.__process_save_as_doc(top))
        submit.grid(row=2, column=0, pady=10, padx=10, columnspan=2)

    def __process_save_as_doc(self, top):
        settings = [top.filter_value.get()]
    ...
    def __command_find_by(self):
        ...
        submit.grid(row=1, column=0, pady=10, padx=10, columnspan=2)
    ...
    def __command_filter_table(self):
        ...
        reset.grid(row=2, column=0, pady=10, padx=10, columnspan=2)
